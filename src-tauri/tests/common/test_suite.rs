//! Shared Aether Desktop verification & reliability test suite.
//!
//! Single source of truth for every test body. It is compiled into both
//! execution modes so expectations can never drift between them:
//! - `src/bin/test_generator.rs` runs the functions sequentially as the
//!   documented standalone verification suite (`cargo run --bin test_generator`).
//! - `tests/generator_test.rs` registers the same functions with the cargo
//!   test harness through thin `#[test]` wrappers.
...
pub fn test_b_missing_tun_triggers_verified_rollback() {
...
pub fn test_c_failed_egress_ip_mismatch_triggers_rollback() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async {
        let mut runner = SingBoxRunner::new();
        let logger = RingBufferLogger::new(100);
...
pub fn test_d_rollback_failure_surfaced_as_critical() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async {
        let mut runner = SingBoxRunner::new();
...
pub fn test_e_persistence_failure_runtime_rollback() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async {
...
pub fn test_f_existing_aether_reuse_decision_path() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async {
        let state = Arc::new(RwLock::new(ConnectionState::Disconnected));
        let logger = aether_desktop_lib::logging::RingBufferLogger::new(10);
...
pub fn test_g_occupied_wrong_port_owner_rejection() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async {
...
pub fn test_h_github_digest_integrity_verification() {
...
pub fn test_i_safe_staging_promotion_restore_and_verification() {
...
pub fn test_j_download_size_and_truncation_guards() {
...
pub fn test_l_aether_scan_mode_startup_deadlines() {
...
pub fn test_m_native_windows_tun_detection_by_ip_and_name() {
...
pub fn test_n_process_elevation_token_check() {
    use aether_desktop_lib::health::HealthProber;
...
pub fn test_p_staged_egress_decision_path_mocked_suite() {
    use aether_desktop_lib::health::HealthProber;
...
pub fn test_q_concurrent_connect_atomic_single_attempt() {
    use aether_desktop_lib::logging::RingBufferLogger;
...
pub fn test_r_op_lock_held_preserves_disconnected_state() {
    use aether_desktop_lib::logging::RingBufferLogger;
    use aether_desktop_lib::models::ConnectionState;
    use aether_desktop_lib::process::orchestrator::ConnectionOrchestrator;
    use parking_lot::RwLock;
...
pub fn test_s_deadline_mapping_strictly_exceeds_upstream_budgets() {
...
pub fn test_t_snapshot_strictly_excludes_identity_and_config_files() {
...
pub fn test_u_snapshot_rollback_atomically_restores_preexisting_lastconn() {
...
pub fn test_v_snapshot_rollback_removes_newly_created_lastconn_on_rollback() {
    use aether_desktop_lib::settings::storage::{lastconn_path, AetherPersistenceSnapshot};
...
pub fn test_w_snapshot_commit_retains_new_lastconn_and_discards_backup() {
    use aether_desktop_lib::settings::storage::{lastconn_path, AetherPersistenceSnapshot};
...
pub fn test_x_tun_teardown_wait_polling_helper() {
    use aether_desktop_lib::health::HealthProber;
...
pub fn test_y_snapshot_restore_failure_is_fatal_to_rollback() {
    use aether_desktop_lib::logging::RingBufferLogger;
    use aether_desktop_lib::models::ConnectionState;
    use aether_desktop_lib::process::ConnectionOrchestrator;
    use aether_desktop_lib::settings::storage::{lastconn_path, AetherPersistenceSnapshot};
...
pub fn test_z_candidate_rtt_processing_works_for_both_stdout_and_stderr() {
    use aether_desktop_lib::logging::RingBufferLogger;
...
pub fn test_aa_restore_deadline_bounded_to_25s() {
    use aether_desktop_lib::models::settings::{
...
pub fn test_ad_save_exported_logs_creates_crlf_log_file() {
...
pub fn test_al_gateway_optimization_evaluation_decision_logic() {
...
pub fn test_am_latency_profile_median_mad_and_insufficient_samples() {
...
pub fn test_an_ring_buffer_logger_10000_capacity_and_eviction() {
...
pub fn test_ao_preexisting_tun_conflict_guard() {
...
pub fn test_ap_singbox_liveness_failure_during_tun_detection() {
...
pub fn test_aq_singbox_liveness_failure_during_staged_egress() {
    use aether_desktop_lib::health::DetectedAdapterInfo;
    use aether_desktop_lib::models::health::CloudflareTrace;
    use aether_desktop_lib::process::runner::verify_router_and_egress_decision_path;
    use std::sync::atomic::{AtomicBool, Ordering};
...
pub fn test_ar_external_aether_guards_abort_optimization_before_disruption() {
    use aether_desktop_lib::logging::RingBufferLogger;
    use aether_desktop_lib::models::ConnectionState;
...
pub fn test_as_tun_stabilization_window_guarantees_adapter_persistence() {
...
pub fn test_at_quick_reconnect_cli_tri_state_and_forced_fresh_scan() {
    use aether_desktop_lib::models::settings::{
...
pub fn test_au_cached_endpoint_reuse_rejected_during_forced_fresh_scan() {
